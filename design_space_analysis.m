...
for i = 1 : length(mission.segments)
    ...
    elseif strcmp(mission.segments{i}.type, 'turn') % Turn segment
        
        [turn1, turn2, min_turn] = turn(pl, wl_grid, wl, k, mission.segments{i}, vehicle, energy);
        yyaxis left;
        ...
        plot(pl, turn1, 'DisplayName', strcat(mission.segments{i}.name, ": First option"));
        plot([min_turn min_turn], [wl(1) wl(end)], 'DisplayName', strcat(mission.segments{i}.name, ": Second option"));
        %plot(pl, turn2) inutil: a curva dá negativa
...
function [WS1_turn, WS2_turn, min] = turn(plf_grid, wl_grid, wl, k, segment, vehicle, energy)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
V = segment.velocity;
n = segment.n;
%[segment_props, ~] = find_by_name(vehicle.segments, segment.name);
Cd0 = 0.0093541996576813058; % generalizar 
q = 0.5 * 1.225 * V^2;

t_w = (1/V)./plf_grid;

WS1_turn = (t_w + sqrt(t_w.^2-(4*n^2*Cd0*k))/(2*n^2/q*k));

WS2_turn = (t_w - sqrt(t_w.^2-(4*n^2*Cd0*k))/(2*n^2/q*k));
...
function wl = turn_constraint_prop(rho, v, cd_0, k)
wl = 0.5 * rho * v^2 * sqrt(cd_0 / k);
